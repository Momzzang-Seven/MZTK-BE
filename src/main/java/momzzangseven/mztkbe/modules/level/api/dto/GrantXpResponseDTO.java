...
public record GrantXpResponseDTO(XpType type, GrantXpStatus status, int grantedXp, int dailyCap) {

  public static GrantXpResponseDTO from(XpType type, GrantXpResult result) {
    return new GrantXpResponseDTO(type, result.status(), result.grantedXp(), result.dailyCap());
  }
}
