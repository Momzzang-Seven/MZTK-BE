 ...
 * signer-wallet preflight (load + active + structural + KMS DescribeKey verify) OUTSIDE any
 * transaction, then hands the validated {@link InternalExecutionSignerGates} to this port.
 * Implementations (and the bean-level {@code TransactionTemplate} wrapper) own the FOR UPDATE
 * intent claim, nonce reservation, sign, broadcast, and persistence steps that must be atomic.
 *
