...
public record WalletRegisterResponseDTO(Long id, String walletAddress, String registeredAt) {
  public static WalletRegisterResponseDTO from(RegisterWalletResult result) {
    return new WalletRegisterResponseDTO(
        result.walletId(), result.walletAddress(), result.registeredAt().toString());
  }
}
