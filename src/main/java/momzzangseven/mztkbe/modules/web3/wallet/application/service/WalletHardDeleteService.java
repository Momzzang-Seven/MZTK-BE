 ...
 * <ul>
 *   <li>Scheduled deletion: UNLINKED wallets after retention period</li>
 *   <li>Cascade deletion: USER_DELETED wallets when users are hard-deleted</li>
 * </ul>
 */
@Slf4j
@Service
...
public class WalletHardDeleteService {
     ...
     *
     * <p>Only deletes UNLINKED wallets (user-initiated unlinking).
     * USER_DELETED wallets are handled by WithdrawalHardDeleteService.
     *
     ...
     *
     * <p>Called by UserHardDeleteEventHandler when it receives user hard-delete event.
     * This ensures wallets are deleted before users, maintaining FK constraints.
     *
    ...
    public int deleteByUserIds(List<Long> userIds) {
        ...
        // Record event
        recordEvent(wallets, props.getRetentionDays(), null, WalletStatus.USER_DELETED, "user_hard_delete_cascade");

        // Batch hard-deletion
        List<Long> walletIds = wallets.stream().map(LoadWalletPort.WalletDeletionInfo::id).toList();
        deleteWalletPort.deleteAllByIdInBatch(walletIds);
        
        log.info("Deleted USER_DELETED wallets for users: walletCount={}, userCount={}", 
                wallets.size(), userIds.size());
        return wallets.size();
    }


    private void recordEvent(List<LoadWalletPort.WalletDeletionInfo> wallets, int retentionDays, Instant cutoff, WalletStatus previousStatus, String action) {
        // Record event
        List<WalletEvent> events = wallets.stream()
                .map(w -> WalletEvent.hardDeleted(
                        w.walletAddress(),
                        w.userId(),
                        previousStatus,
                        Map.of(
