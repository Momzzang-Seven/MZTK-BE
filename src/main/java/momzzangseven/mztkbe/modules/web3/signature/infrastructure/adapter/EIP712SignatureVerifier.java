 ...
 * 1. Structure TypedData (Domain + Message)
 * 2. Calculate Domain Separator
 * 3. Calculate Message Hash
 * 4. Calculate EIP-712 Digest: keccak256("\x19\x01" ‖ domainSeparator ‖ messageHash)
 * 5. Recover signer address using ecrecover
...
public class EIP712SignatureVerifier implements VerifySignaturePort {
    ...
    private void validateInputs(
            String message, String nonce, String signature, String address) {
        ...
        if (signature == null || !signature.matches("^0x[0-9a-fA-F]{130}$")) {
            throw new IllegalArgumentException(
                    "Invalid signature format (expected: 0x + 130 hex chars)");
        }
     ...
     *
     * <p>Web3j's StructuredDataEncoder requires JSON string input.
     * This method converts our domain models to the exact JSON format required by EIP-712.
     *
