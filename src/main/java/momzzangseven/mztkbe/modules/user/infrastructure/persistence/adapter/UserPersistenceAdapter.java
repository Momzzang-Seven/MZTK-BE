 ...
 * - Implements OUTPUT PORTS defined by the application layer
 * - Translates between Domain Model (User) and Infrastructure Model (UserEntity)
 *
 * Responsibilities:
 * - Execute database operations via UserJpaRepository
 * - Convert UserEntity ↔ User (Domain Model)
...
public class UserPersistenceAdapter implements LoadUserPort, SaveUserPort {
    ...
    public Optional<User> loadUserByWalletAddress(String walletAddress) {
        ...
        return userJpaRepository.findByWalletAddress(walletAddress)
                .map(this::mapToDomain);
    }
    ...
    /**
     * Convert User (Domain) to UserEntity (Infrastructure).
    ...
    /**
     * Update existing UserEntity from User (Domain).
     * Used for updating entities to preserve JPA managed state.
     */
