 ...
 *
 * <p>Responsibilities:
 * - Implement LoadRefreshTokenPort and SaveRefreshTokenPort
 * - Translate between RefreshToken (domain) and RefreshTokenEntity (JPA)
 * - Hash token values before storing (security)
 * - Use tokenHash for database queries
 *
 * <p>Security Design:
 * - Stores SHA-256 hash in database, not plain token
 * - Original token value is only kept in memory (domain model)
...
public class RefreshTokenPersistenceAdapter implements LoadRefreshTokenPort, SaveRefreshTokenPort {
    ...
    private RefreshToken mapToDomain(RefreshTokenEntity entity, String tokenValue) {
        return RefreshToken.builder()
                ...
                .tokenValue(entity.getTokenHash())
    ...
    /**
     * Convert RefreshToken (Domain) to RefreshTokenEntity (Infrastructure).
    ...
    /**
     * Update existing managed entity from domain model.
     * Uses setters to maintain JPA managed state.
     */
