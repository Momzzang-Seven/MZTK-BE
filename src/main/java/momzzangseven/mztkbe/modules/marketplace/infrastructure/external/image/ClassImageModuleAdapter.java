package momzzangseven.mztkbe.modules.marketplace.infrastructure.external.image;
...
import momzzangseven.mztkbe.modules.marketplace.application.dto.GetClassDetailResult.ImageInfo;
 ...
 *
 * <p>
 * Implements both {@link UpdateClassImagesPort} and
 * {@link LoadClassImagesPort}. Follows the
 * exact same pattern as
 * {@code post.infrastructure.external.image.ImageModuleAdapter}.
 *
 * <ul>
 * <li>{@code updateImages} delegates to {@link UpsertImagesByReferenceUseCase}
 * using {@link
 * ImageReferenceType#MARKET_CLASS} (the virtual aggregate type).
 * <li>{@code loadImages} delegates to {@link GetImagesByReferenceUseCase} to
 * get both THUMB and
 * DETAIL images.
 * <li>{@code loadThumbnailKeys} delegates to
 * {@link GetImagesByReferencesUseCase} for batch
...
public class ClassImageModuleAdapter implements UpdateClassImagesPort, LoadClassImagesPort {
  ...
  public ClassImages loadImages(Long classId) {
    GetImagesByReferenceResult result = getImagesByReferenceUseCase.execute(
  ...
  public Map<Long, String> loadThumbnailKeys(List<Long> classIds) {
    GetImagesByReferencesResult result = getImagesByReferencesUseCase.execute(
   ...
   *
   * <p>
   * The image module returns items sorted by {@code img_order} ascending. The
   * thumbnail is
   * always the item with the lowest order (conventionally {@code img_order = 1}).
   * Rather than
   * relying on the fragile object-key path prefix, we treat the first item in the
   * sorted list as
   * the thumbnail ({@code firstItem == true}).
   *
   * <p>
   * If the image module ever changes its sort contract, update this method
   * accordingly.
   */
