...
public record StepUpRequestDTO(String password, String authorizationCode) {
  ...
  /** Convert this request to an application-layer command using the authenticated user's ID. */
  public StepUpCommand toCommand(Long userId) {
    return StepUpCommand.of(userId, password, authorizationCode);
  }
}
