...
BEGIN
    ...
    IF EXISTS (
        ...
        FROM pg_class c
        JOIN pg_index i ON i.indexrelid = c.oid
        WHERE c.oid = to_regclass('uk_web3_tx_eip7702_authority_nonce')
          AND (NOT i.indisvalid OR NOT i.indisready)
    ) THEN
        DROP INDEX uk_web3_tx_eip7702_authority_nonce;
    END IF;
    ...
    IF EXISTS (
        ...
        FROM pg_class c
        JOIN pg_index i ON i.indexrelid = c.oid
        WHERE c.oid = to_regclass('uk_web3_tx_non_reward_eip1559_sender_nonce')
          AND (NOT i.indisvalid OR NOT i.indisready)
    ) THEN
        DROP INDEX uk_web3_tx_non_reward_eip1559_sender_nonce;
    END IF;
