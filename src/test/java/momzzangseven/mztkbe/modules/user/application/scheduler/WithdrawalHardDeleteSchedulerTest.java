...
class WithdrawalHardDeleteSchedulerTest {
  ...
  void run_noBatch_doesNotLog() {
    given(hardDeleteService.runBatch(any())).willReturn(0);

    scheduler.run();

    verify(hardDeleteService, times(1)).runBatch(any());
  }

  @Test
  @DisplayName("run() - 배치 삭제 후 0 반환 시 루프 종료, 완료 로그")
  void run_batchCompletedWithDeletions_logs() {
    given(hardDeleteService.runBatch(any())).willReturn(30).willReturn(0);

    scheduler.run();

    verify(hardDeleteService, times(2)).runBatch(any());
  }
}
