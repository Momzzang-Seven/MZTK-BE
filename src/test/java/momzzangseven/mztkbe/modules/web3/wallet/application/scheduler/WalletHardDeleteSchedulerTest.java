...
@DisplayName("WalletHardDeleteScheduler 단위 테스트")
class WalletHardDeleteSchedulerTest {

  @Mock private WalletHardDeleteService walletHardDeleteService;

  @InjectMocks private WalletHardDeleteScheduler scheduler;

  @Test
  @DisplayName("run() - 첫 배치에서 0 반환 시 루프 즉시 종료, 로그 없음")
  void run_noBatch_doesNotLog() {
    given(walletHardDeleteService.runBatch(any())).willReturn(0);

    scheduler.run();

    verify(walletHardDeleteService, times(1)).runBatch(any());
  }

  @Test
  @DisplayName("run() - 배치 삭제 후 0 반환 시 루프 종료, 완료 로그")
  void run_batchCompletedWithDeletions_logs() {
    given(walletHardDeleteService.runBatch(any())).willReturn(20).willReturn(0);

    scheduler.run();

    verify(walletHardDeleteService, times(2)).runBatch(any());
  }
}
