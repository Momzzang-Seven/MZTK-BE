...
@DisplayName("ImageAnswerOrphanCleanupService 단위 테스트")
class ImageAnswerOrphanCleanupServiceTest {

  @Mock private LoadImagePort loadImagePort;
  @Mock private DeleteImagePort deleteImagePort;
  @Mock private ImageAnswerOrphanCleanupProperties props;

  @InjectMocks private ImageAnswerOrphanCleanupService cleanupService;
  ...
  @Test
  @DisplayName("고아 answer-linked 이미지가 없으면 0을 반환하고 unlink를 호출하지 않는다")
  void runBatch_noOrphanImages_returnsZero() {
    given(loadImagePort.findOrphanAnswerImages(100)).willReturn(List.of());
  ...
  @Test
  @DisplayName("고아 answer-linked 이미지를 일괄 unlink 한다")
  void runBatch_unlinksOrphanImages() {
    given(loadImagePort.findOrphanAnswerImages(100)).willReturn(List.of(image(10L), image(11L)));
