...
`docs.local/load_test/phaseN/results/` 아래에:
- `RESULT_<유형>.md` — 결과 문서 (`RESULT_BP.md` / `RESULT_LOAD.md` / `RESULT_ENDU.md`)
...
- k6 result JSON 경로 — 보통 `phaseN/results/<유형>_<날짜>.json`
- 출력 phase — 결과를 둘 `phaseN/results/` 디렉토리
...
```
차트가 `phaseN/results/charts/` 에 생성된다. 팔레트에 없는 자원이 병목이면 §3 메트릭으로
...
phase 의 results 디렉토리에 자체 `RESULT_GUIDE.md` 가 있으면 그 템플릿을, 없으면 번들된
`assets/RESULT_GUIDE.md` 를 기준으로 `RESULT_<유형>.md` 를 쓴다(같은 유형 재측정이면
기존 파일 맨 위에 회차 섹션 추가). 판정 기준은 **해당 phase 의 판정 가이드가 있으면 1순위로**
...
새 `RESULT_<유형>.md` 를 만들었으면 `RESULT_GUIDE.md` 의 유형↔파일 매핑 표와, 관련
INSIGHT_GUIDE 의 결과 파일 참조를 새 파일명으로 갱신한다. stale 참조가 남지 않게 한다.
